...
*   🚀 **被动 SSR 提取**: 直接解析网页 `window.__INITIAL_STATE__` 数据，获取服务端原始数据，速度快且数据完整。
...
*   🔄 **增量更新**: 自动去重（2026年增强：CSV/JSON支持跨运行去重），支持断点续爬。
...
├── selectors.json           # [配置] CSS选择器配置
├── requirements.txt         # [依赖] 最小版本约束
├── requirements-lock.txt    # [依赖] 锁定版本（推荐）
